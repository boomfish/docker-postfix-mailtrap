...
# Create user for trap mailbox?
if [ "$mailbox_user_create" = "true" ]; then
  useradd -u $mailbox_uid -m $mailbox_username
fi
...
# Route all external mail to a single local mailbox
echo "/^.*@.*\..*\$/ ${mailbox_username}@${maildomain}" > /etc/postfix/canonical
